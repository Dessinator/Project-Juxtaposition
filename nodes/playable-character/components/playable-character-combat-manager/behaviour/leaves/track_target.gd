...
func tick(actor: Node, blackboard: BHBlackboard) -> int:
	...
	var position = blackboard.get_value("tracked_target_position")
	...
	rotation_tween.tween_property(camera._camera_rotation_pivot, "global_rotation:y", camera_node.global_rotation.y, 0.1)
