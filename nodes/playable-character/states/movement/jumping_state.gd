...
func _handle_jumping(current_velocity: Vector3, direction: Vector3, delta: float) -> Vector3:
	...
	if direction.is_zero_approx():
		velocity = current_velocity.move_toward(current_velocity.normalized() + (Vector3.DOWN * _gravity), _acceleration * delta)
	else:
		velocity = current_velocity.move_toward(direction + (Vector3.DOWN * _gravity), _acceleration * delta)
