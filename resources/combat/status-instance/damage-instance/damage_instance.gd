...
@export var time_to_interrupt: float = 0.5
...
func duplicate_instance() -> DamageInstance:
	...
	damage_instance.source = self.source
	damage_instance.base_damage = self.base_damage
	damage_instance.can_dodge = self.can_dodge
	damage_instance.can_parry = self.can_parry
	damage_instance.time_to_interrupt = self.time_to_interrupt
